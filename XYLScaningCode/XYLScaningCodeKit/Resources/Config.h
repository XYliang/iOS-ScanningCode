...
typedef NS_ENUM(NSInteger, XYLScaningViewCoverMode) {
    XYLScaningViewCoverModeClear,
    XYLScaningViewCoverModeNormal, //默认
    XYLScaningViewCoverModeBlur,
};

typedef NS_ENUM(NSInteger, XYLScaningViewShapeMode) {
    XYLScaningViewShapeModeSquare, //默认
    XYLScaningViewShapeModeRound,
   
};

typedef NS_ENUM(NSInteger, XYLScaningLineMoveMode) {
    XYLScaningLineMoveModeDown, //默认
    XYLScaningLineMoveModeUpAndDown,
    XYLScaningLineMoveModeNone,
 
};

typedef NS_ENUM(NSInteger, XYLScaningLineMode) {
    XYLScaningLineModeNone, //什么都没有
    XYLScaningLineModeDeafult, //默认
    XYLScaningLineModeImge,  //以一个图为扫描线，微信，百度
    XYLScaningLineModeGrid, //类似京东
};

typedef NS_ENUM(NSInteger, XYLScaningWarningTone) {
    XYLScaningWarningToneSound, //声音
    XYLScaningWarningToneVibrate, //震动
    XYLScaningWarningToneSoundAndVibrate, //声音和振动
};
...
#define LineColor Color(225,0,225,.2) //扫描线的颜色，RGBA,用户可以定义任何颜色（紫色）
